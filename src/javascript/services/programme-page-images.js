...
class ProgrammePageImagesService {
  constructor() {
  ...
  uploadImages(assets, id) {
    ...
    assets.map((asset, i) => {
      ...
      axios({
        url: `${apiConfig.apiUrl}/programme/${id}/page-images`,
        method: 'POST',
        headers: {
  ...
  patchUploadImages(resources, id) {
    ...
    const endpoint = `${apiConfig.apiUrl}/programme/${id}/page-images`
    Object.keys(resources).map((key) => {
      const resource = resources[key]
      if (resource.file) {
        axios({
...
const service = new ProgrammePageImagesService

...
function createSourceWithoutTags(source, clientName) {
  /*
  ...
  for (let i = 0; i < sourceLines.length; i++) {
    ...
    if (line.search(regionStartTag) > -1) {
      ...
      if (regionTagOrder.length === 0) {
        ...
        //nested - test tag is valid within surrounding region
        const surroundingRegionTags = regionTagOrder[regionTagOrder.length - 1]
        const nestedTagsExistInSurroundingRegion = startLineTags.reduce(
          (acc, curr) => {
            if (!acc) return acc
      ...
      if (regionTagOrder.length === 0) {
        error = `ERROR: (${i +
          1}): Unbalanced region tags, found #endregion without start #region`
        break
      } else {
        regionTagOrder.pop()
      }
    ...
    if (!includedRegion.length) {
      error = `ERROR: (${i +
        1}): Unbalanced region tags, unable to decipher if code should be included or not`
      break
    }
  ...
  if (!error && regionTagOrder.length !== 0) {
    // check final #endregion exists
